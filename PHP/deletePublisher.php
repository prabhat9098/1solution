...
if ($_SERVER["REQUEST_METHOD"] == "DELETE") {
    ...
    if (isset($_GET["id"])) {
        ...
        $delete_sql = "DELETE FROM publishers WHERE PublisherID = ?";
        ...
        if ($stmt->execute()) {
            ...
            $response = array("success" => true, "message" => "Publisher deleted successfully");
            echo json_encode($response);
        } else {
            // Error deleting author data
            http_response_code(400);
            $response = array("success" => false, "message" => "Error deleting Publisher: " . $stmt->error);
            echo json_encode($response);
        }
